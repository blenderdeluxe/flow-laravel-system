...
class Flow
{
    ...
    private function flow_pack()
    {
        ...
        if (!$hConcepto) {
            $hConcepto = htmlentities($concepto, ENT_COMPAT | ENT_HTML401, 'UTF-8');
        }

        if (!$hConcepto) {
            $hConcepto = htmlentities($concepto, ENT_COMPAT | ENT_HTML401, 'ISO-8859-1');
        }

        if (!$hConcepto) {
            $hConcepto = "Orden de Compra $orden_compra";
        }
